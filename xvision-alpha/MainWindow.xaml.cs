...
namespace Microsoft.Samples.Kinect.BodyBasics
{
    ...
    public partial class MainWindow : Window, INotifyPropertyChanged
    {
        ...
        private readonly Brush headBrush = new SolidColorBrush(Color.FromArgb(255, 0, 255, 0));
        ...
        private ScreenProjector screenMapper = null;
        ...
        private void DrawImage(Point a, Point b, ImageSource image, DrawingContext dc)
        {
            double angle = 90 + Math.Atan2(b.Y - a.Y, b.X - a.X) * 180 / Math.PI;
            double height = Math.Sqrt(Math.Pow(b.X - a.X, 2) + Math.Pow(b.Y - a.Y, 2));
            double width = (image.Width / image.Height) * height;

            var rotation = new RotateTransform(angle);
            var translation = new TranslateTransform(b.X, b.Y);
            dc.PushTransform(translation);
            dc.PushTransform(rotation);
            dc.DrawImage(image, new Rect(0 - width / 2, 0, width, height));
            dc.Pop();
            dc.Pop();
        }
