...
//	// Public bool to determine whether to allow turnaround-mode
	...
	public AllowedRotations allowedHandRotations = AllowedRotations.All;
...
//				dwFlags |= KinectInterop.FrameSource.TypeAudio;
		...
		// PLAYBACK mode initialization buffer
		if (enablePlaybackMode) {
			initializeBodyFrameBuffer();
		}
	}
		...
		if (enablePlaybackMode && isBodyFrameBufferInitialized && isPlaybackPlaying()) {
			if(bodyFrameCount > 0 && bodyFrameCount < bodyFrameBufferSize && lastPlayTime + timeBetweenTwoPlayback < Time.time)
			{
				if(!enableReversePlayback)
				{
					...
					playBackStreamReaderIndex = playBackStreamReaderIndex % bodyFrameCount;
				}
				else
				{
